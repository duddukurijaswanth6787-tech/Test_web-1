...
export const AiPromptModal: React.FC<AiPromptModalProps> = ({ client, onClose }) => {
  ...
  const planName = client.subscription?.planName || "Starter Plan";
  const maxImages = client.subscription?.maxImages || 30;
  const maxStorageMb = client.subscription?.maxStorageBytes
    ? Math.round(client.subscription.maxStorageBytes / (1024 * 1024))
    : 1200;
...
- [ ] 2. Open \`/admin\` -> Verify quota bar shows ${maxImages} photos limit.
